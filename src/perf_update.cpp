#include "perf_common.hpp"

using namespace psds;

#define EXE_PERF_UPDATE(TREE, i)                                            \
    {                                                                       \
        TREE tree;                                                          \
        uint64_t n = uint64_t(1) << benchmarking::logs[i];                  \
        std::cout << "### n = " << n << std::endl;                          \
        {                                                                   \
            std::vector<int64_t> input(n);                                  \
            std::generate(input.begin(), input.end(),                       \
                          [&] { return distr_values.gen(); });              \
            tree.build(input.data(), n);                                    \
        }                                                                   \
        essentials::uniform_int_rng<uint32_t> distr_queries(                \
            0, n - 1, benchmarking::query_seed);                            \
        std::generate(queries.begin(), queries.end(), [&] {                 \
            return std::make_pair(distr_queries.gen(), distr_values.gen()); \
        });                                                                 \
        PERF_UPDATE                                                         \
        json += std::to_string(avg_ns_query) + ',';                         \
    }

template <typename Node>
void perf_segment_tree_simd_node16() {
    essentials::uniform_int_rng<int64_t> distr_values(-100, 100,
                                                      benchmarking::value_seed);
    std::vector<std::pair<uint32_t, int64_t>> queries(
        benchmarking::num_queries);
    std::string json("{\"type\":\"" + GEN_TYPE_NODE16(0, Node)::name() +
                     "\", \"timings\":[");

    EXE_PERF_UPDATE(GEN_TYPE_NODE16(0, Node), 0)
    EXE_PERF_UPDATE(GEN_TYPE_NODE16(1, Node), 1)
    EXE_PERF_UPDATE(GEN_TYPE_NODE16(2, Node), 2)
    EXE_PERF_UPDATE(GEN_TYPE_NODE16(3, Node), 3)
    EXE_PERF_UPDATE(GEN_TYPE_NODE16(4, Node), 4)
    EXE_PERF_UPDATE(GEN_TYPE_NODE16(5, Node), 5)
    EXE_PERF_UPDATE(GEN_TYPE_NODE16(6, Node), 6)
    EXE_PERF_UPDATE(GEN_TYPE_NODE16(7, Node), 7)
    EXE_PERF_UPDATE(GEN_TYPE_NODE16(8, Node), 8)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(9, Node), 9)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(10, Node), 10)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(11, Node), 11)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(12, Node), 12)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(13, Node), 13)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(14, Node), 14)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(15, Node), 15)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(16, Node), 16)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(17, Node), 17)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(18, Node), 18)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(19, Node), 19)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(20, Node), 20)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(21, Node), 21)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(22, Node), 22)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE16(23, Node), 23)

    json.pop_back();
    json += "]}";
    std::cerr << json << std::endl;
}

template <typename Node>
void perf_segment_tree_simd_node64() {
    essentials::uniform_int_rng<int64_t> distr_values(-100, 100,
                                                      benchmarking::value_seed);
    std::vector<std::pair<uint32_t, int64_t>> queries(
        benchmarking::num_queries);
    std::string json("{\"type\":\"" + GEN_TYPE_NODE64(0, Node)::name() +
                     "\", \"timings\":[");

    EXE_PERF_UPDATE(GEN_TYPE_NODE64(0, Node), 0)
    EXE_PERF_UPDATE(GEN_TYPE_NODE64(1, Node), 1)
    EXE_PERF_UPDATE(GEN_TYPE_NODE64(2, Node), 2)
    EXE_PERF_UPDATE(GEN_TYPE_NODE64(3, Node), 3)
    EXE_PERF_UPDATE(GEN_TYPE_NODE64(4, Node), 4)
    EXE_PERF_UPDATE(GEN_TYPE_NODE64(5, Node), 5)
    EXE_PERF_UPDATE(GEN_TYPE_NODE64(6, Node), 6)
    EXE_PERF_UPDATE(GEN_TYPE_NODE64(7, Node), 7)
    EXE_PERF_UPDATE(GEN_TYPE_NODE64(8, Node), 8)
    EXE_PERF_UPDATE(GEN_TYPE_NODE64(9, Node), 9)
    EXE_PERF_UPDATE(GEN_TYPE_NODE64(10, Node), 10)
    EXE_PERF_UPDATE(GEN_TYPE_NODE64(11, Node), 11)
    EXE_PERF_UPDATE(GEN_TYPE_NODE64(12, Node), 12)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE64(13, Node), 13)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE64(14, Node), 14)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE64(15, Node), 15)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE64(16, Node), 16)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE64(17, Node), 17)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE64(18, Node), 18)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE64(19, Node), 19)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE64(20, Node), 20)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE64(21, Node), 21)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE64(22, Node), 22)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE64(23, Node), 23)

    json.pop_back();
    json += "]}";
    std::cerr << json << std::endl;
}

template <typename Node>
void perf_segment_tree_simd_node256() {
    essentials::uniform_int_rng<int64_t> distr_values(-100, 100,
                                                      benchmarking::value_seed);
    std::vector<std::pair<uint32_t, int64_t>> queries(
        benchmarking::num_queries);
    std::string json("{\"type\":\"" + GEN_TYPE_NODE256(0, Node)::name() +
                     "\", \"timings\":[");

    EXE_PERF_UPDATE(GEN_TYPE_NODE256(0, Node), 0)
    EXE_PERF_UPDATE(GEN_TYPE_NODE256(1, Node), 1)
    EXE_PERF_UPDATE(GEN_TYPE_NODE256(2, Node), 2)
    EXE_PERF_UPDATE(GEN_TYPE_NODE256(3, Node), 3)
    EXE_PERF_UPDATE(GEN_TYPE_NODE256(4, Node), 4)
    EXE_PERF_UPDATE(GEN_TYPE_NODE256(5, Node), 5)
    EXE_PERF_UPDATE(GEN_TYPE_NODE256(6, Node), 6)
    EXE_PERF_UPDATE(GEN_TYPE_NODE256(7, Node), 7)
    EXE_PERF_UPDATE(GEN_TYPE_NODE256(8, Node), 8)
    EXE_PERF_UPDATE(GEN_TYPE_NODE256(9, Node), 9)
    EXE_PERF_UPDATE(GEN_TYPE_NODE256(10, Node), 10)
    EXE_PERF_UPDATE(GEN_TYPE_NODE256(11, Node), 11)
    EXE_PERF_UPDATE(GEN_TYPE_NODE256(12, Node), 12)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE256(13, Node), 13)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE256(14, Node), 14)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE256(15, Node), 15)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE256(16, Node), 16)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE256(17, Node), 17)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE256(18, Node), 18)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE256(19, Node), 19)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE256(20, Node), 20)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE256(21, Node), 21)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE256(22, Node), 22)
    // EXE_PERF_UPDATE(GEN_TYPE_NODE256(23, Node), 23)

    json.pop_back();
    json += "]}";
    std::cerr << json << std::endl;
}

template <typename Tree>
void perf_test() {
    essentials::uniform_int_rng<int64_t> distr_values(-100, 100,
                                                      benchmarking::value_seed);
    std::vector<std::pair<uint32_t, int64_t>> queries(
        benchmarking::num_queries);

    std::string json("{\"type\":\"" + Tree::name() + "\", \"timings\":[");

    EXE_PERF_UPDATE(Tree, 0)
    EXE_PERF_UPDATE(Tree, 1)
    EXE_PERF_UPDATE(Tree, 2)
    EXE_PERF_UPDATE(Tree, 3)
    EXE_PERF_UPDATE(Tree, 4)
    EXE_PERF_UPDATE(Tree, 5)
    EXE_PERF_UPDATE(Tree, 6)
    EXE_PERF_UPDATE(Tree, 7)
    EXE_PERF_UPDATE(Tree, 8)
    EXE_PERF_UPDATE(Tree, 9)
    EXE_PERF_UPDATE(Tree, 10)
    EXE_PERF_UPDATE(Tree, 11)
    EXE_PERF_UPDATE(Tree, 12)
    // EXE_PERF_UPDATE(Tree, 13)
    // EXE_PERF_UPDATE(Tree, 14)
    // EXE_PERF_UPDATE(Tree, 15)
    // EXE_PERF_UPDATE(Tree, 16)
    // EXE_PERF_UPDATE(Tree, 17)
    // EXE_PERF_UPDATE(Tree, 18)
    // EXE_PERF_UPDATE(Tree, 19)
    // EXE_PERF_UPDATE(Tree, 20)
    // EXE_PERF_UPDATE(Tree, 21)
    // EXE_PERF_UPDATE(Tree, 22)
    // EXE_PERF_UPDATE(Tree, 23)

    json.pop_back();
    json += "]}";
    std::cerr << json << std::endl;
}

int main(int argc, char** argv) {
    MAIN return 0;
}
